...
class AgregarAmigoUseCaseTest {
  ...
  @BeforeEach void setUp() { useCase = new AgregarAmigoUseCase(repo); }
  @Test @DisplayName("debe llamar a addFriendship")
  void debeLlamarAAddFriendship() {
    useCase.execute("u1", "u2");
    verify(repo).addFriendship("u1", "u2");
  }
  @Test @DisplayName("debe lanzar excepcion con ids iguales")
  void debeLanzarConMismosIds() {
    assertThrows(IllegalArgumentException.class, () -> useCase.execute("u1", "u1"));

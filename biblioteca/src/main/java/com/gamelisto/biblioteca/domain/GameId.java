...
public final class GameId {
  ...
  private GameId(Long value) {
    ...
    if (value <= 0) {
      throw new DomainException("El id del juego debe ser un entero positivo");
    }
    this.value = value;
  }
  ...
  public static GameId fromString(String value) {
    if (value == null || value.trim().isEmpty()) {
      throw new DomainException("El id del juego no puede ser nulo o vacío");
    }
    try {
      return new GameId(Long.parseLong(value));
    } catch (NumberFormatException e) {
      throw new DomainException("Formato de id de juego inválido: " + value, e);
    }
  }

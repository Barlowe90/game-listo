...
public class ScheduledSyncJob {
    ...
    private volatile boolean gamesSyncCompleted = false;

    @Scheduled(cron = "0,30 * * * * *") // Cada 30 segundos
    public void syncGamesIncremental() {
        logger.info("===== Iniciando sincronización automática de juegos =====");

        if (gamesSyncCompleted) {
            return;
        }
        ...
        try {
            ...
            if (result.totalSynced() == 0) {
                gamesSyncCompleted = true;
                logger.info("===== Sincronización de juegos finalizada. No se programarán más ejecuciones útiles =====");
            }

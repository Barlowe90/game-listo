...
Implementa el ciclo de vida completo del usuario: registro, verificación de email, gestión de perfil, cambio y restablecimiento de contraseña.

> **Nota:** Este microservicio gestiona la verificación de email y restablecimiento de contraseña mediante tokens con expiración de 24 horas.  
...
- Login/Logout (gestión de sesiones JWT → `auth-service`)
- Validación de tokens JWT (API Gateway)
...
docker compose up --build usuarios-service postgres
```
...
```bash
# Health check
curl http://localhost:8080/v1/usuarios/health
...
| `JWT_SECRET` | - | Secret para validación JWT (futuro) |
| `TOKEN_EXPIRATION_HOURS` | `24` | Expiración de tokens de verificación |

...
@Tag(name = "Autenticación", description = "Registro, login, tokens, verificación de email y recuperación de cuenta")
public class AuthController {
    ...
    @Operation(summary = "Registrar nuevo usuario", description = "Crea una nueva cuenta de usuario en estado PENDIENTE_DE_VERIFICACION. "
            + "Envía un email con token de verificación (válido 24h).")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Usuario creado exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UsuarioResponse.class))),
            @ApiResponse(responseCode = "400", description = "Datos inválidos o email/username ya registrados")
    })
    ...
    @Operation(summary = "Verificar email", description = "Activa la cuenta del usuario mediante el token enviado por email. "
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Token inválido, expirado o usuario ya verificado")
    })
    @PostMapping("/verify-email")
    public ResponseEntity<Void> verificarEmail(
            @Parameter(description = "Token de verificación", required = true) @Valid @RequestBody VerificarEmailRequest request) {
    ...
    @Operation(summary = "Reenviar email de verificación", description = "Genera y envía un nuevo token de verificación al email del usuario. "
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Email no registrado o usuario ya verificado")
    })
    ...
    @Operation(summary = "Solicitar restablecimiento de contraseña", description = "Genera y envía un token de restablecimiento al email (válido 1h). "
            + "Si el email no existe, retorna 200 OK por seguridad (no revela si el email está registrado).")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Si el email existe, se envió el token de restablecimiento")
    })
    ...
    @Operation(summary = "Restablecer contraseña", description = "Cambia la contraseña del usuario usando el token de restablecimiento. "
    ...
    @Operation(summary = "Login de usuario", description = "Autentica al usuario con email y contraseña. Retorna access token JWT (15 min) y refresh token UUID (7 días).")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Credenciales inválidas o usuario no activo"),
            @ApiResponse(responseCode = "400", description = "Datos de entrada inválidos")
    })
    @PostMapping("/login")
    public ResponseEntity<AuthResponse> login(
            @Parameter(description = "Credenciales de login", required = true) @Valid @RequestBody LoginRequest request) {
    ...
    @Operation(summary = "Renovar access token", description = "Genera un nuevo access token usando un refresh token válido. "
            + "Implementa Refresh Token Rotation: el refresh token antiguo se revoca automáticamente. "
            + "Requiere autenticación.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Tokens renovados exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AuthResponse.class))),
            @ApiResponse(responseCode = "401", description = "Refresh token inválido, revocado o expirado"),
            @ApiResponse(responseCode = "400", description = "Datos de entrada inválidos")
    })
    @PreAuthorize("isAuthenticated()")
    @PostMapping("/refresh")
    public ResponseEntity<AuthResponse> refresh(
            @Parameter(description = "Refresh token válido", required = true) @Valid @RequestBody RefreshTokenRequest request) {
    ...
    @Operation(summary = "Logout de usuario", description = "Cierra la sesión revocando el refresh token. "
    ...
    public ResponseEntity<Void> logout(
            @Parameter(description = "Tokens a revocar", required = true) @Valid @RequestBody LogoutRequest request) {
    ...
    @Operation(summary = "Obtener perfil autenticado", description = "Retorna los datos del usuario autenticado. "
            + "El userId se extrae del access token JWT (claim 'sub') por el API Gateway y se envía en el header X-User-Id. "
            + "Requiere autenticación.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Perfil obtenido exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UsuarioResponse.class))),
            @ApiResponse(responseCode = "401", description = "No autenticado - Header X-User-Id ausente"),
            @ApiResponse(responseCode = "404", description = "Usuario no encontrado")
    })
    @PreAuthorize("isAuthenticated()")
    @GetMapping("/me")
    public ResponseEntity<UsuarioResponse> getAuthenticatedProfile(
            @Parameter(description = "User ID extraído del JWT por el Gateway", required = true, hidden = true) @RequestHeader(value = "X-User-Id", required = false) String userId) {

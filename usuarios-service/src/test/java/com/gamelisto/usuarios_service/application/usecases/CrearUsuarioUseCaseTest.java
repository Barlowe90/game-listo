...
class CrearUsuarioUseCaseTest {
    ...
    void debeLanzarExcepcionSiUsernameYaExiste() {
        ...
        // Act & Assert
        UsernameYaExisteException exception = assertThrows(
            UsernameYaExisteException.class,
            () -> crearUsuarioUseCase.execute(command)
        );
    ...
    void debeVerificarExistenciaConValoresNormalizados() {
        ...
        // Assert
        verify(repositorioUsuarios).existsByUsername(argThat(username -> 
            username.value().equals("Usuario123") // Username NO se normaliza a minúsculas
        ));
        verify(repositorioUsuarios).existsByEmail(argThat(email -> 
            email.value().equals("email@test.com") // Email SÍ se normaliza
        ));
    }
}
